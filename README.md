...
- **Текстовые сообщения**: Интеллектуальная обработка текстовых запросов с использованием RAG (Retrieval-Augmented Generation)
- **Голосовые сообщения**: Автоматическая транскрибация голосовых сообщений с помощью OpenAI Whisper и их последующая обработка как текстовых
- **База знаний**: Поиск релевантной информации в базе знаний для генерации точных ответов
- **Интеграция с Amo CRM**: Автоматическое создание лидов с поддержкой тегов
- **Управление контактами**: Сбор контактной информации пользователей
- **Административная панель**: Управление базой знаний через административного бота
...
| `OPENAI_API_KEY` | API ключ для OpenAI (обязателен, если `USE_LOCAL_MODEL=false` или для транскрибации голосовых сообщений) |
